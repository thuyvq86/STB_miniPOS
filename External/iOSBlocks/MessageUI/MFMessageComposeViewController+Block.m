               ...
               onFinish:(ComposeFinishedBlock)finished
{
    ...
    //MARK: Nam Nguyen fixed
    //if ([self canSendSubject] && subject) {
    // These checks basically make sure it's an MMS capable device with iOS7
    if (subject && [MFMessageComposeViewController respondsToSelector:@selector(canSendSubject)] && [MFMessageComposeViewController canSendSubject]){
    ...
    // These checks basically make sure it's an MMS capable device with iOS7
    if(attachments.count > 0 && [MFMessageComposeViewController respondsToSelector:@selector(canSendAttachments)] && [MFMessageComposeViewController canSendAttachments])
    {
