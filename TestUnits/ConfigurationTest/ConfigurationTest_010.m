...
-(void)backgroundPrintBitmap {
    ...
    [self beginTimeMeasure];  
    iBPResult retValue = [self.configurationChannel iBPPrintBitmap:self.selectedBitmap size:CGSizeMake([self.preferredWidth.text floatValue], [self.preferredHeight.text floatValue]) alignment:UITextAlignmentLeft];
    float totalTime = [self endTimeMeasure];
