...
-(void)confSerialData:(NSData *)data incoming:(BOOL)isIncoming {
    NSLog(@"%s [DATA (%@), LENGTH: %d]\n\t%@", __FUNCTION__, ((isIncoming == YES) ? @"iSMP -> iPhone" : @"iPhone -> iSMP"), [data length], [data hexDump]);
}
...
-(void)shouldCutPaper {
    ...
    self.bitmapReceipt.textAlignment = NSTextAlignmentCenter;
...
- (void)receivedRequest:(NSString *)request{
    ...
    NSInteger i = 0;
    for (i = 0; i < _delegateCount; i++) {
        if ([(NSObject *)_delegateList[i] respondsToSelector:@selector(receivedRequest:)]) {
            [(id<ISMPControlManagerDelegate>)_delegateList[i] receivedRequest:request];
        }
    }
}
