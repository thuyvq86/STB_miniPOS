...
@synthesize testGroups, labelApiVersion, labelApplicationBuildTime, labelAppVersion, labelCompanionUsed;
...
-(void)monitorBatteryHelper {
	...
	NSString * str = [NSString stringWithFormat:@"%@;%f;%ld;%ld\r\n", time, iPhoneBatteryLevel, (long)iSpmBatteryLevel, (long)acLine];
...
- (void)viewDidUnload {
    [self setLabelCompanionUsed:nil];
...
-(void)viewWillAppear:(BOOL)animated {
    ...
    NSString *Companion = [prefs stringForKey:@"IngenicoCompanionInUse"];
    [ICISMPDevice setWantedDevice:Companion];
    if(Companion != nil)
    {
        labelCompanionUsed.text = [NSString stringWithFormat:@"Companion used: %@", Companion];
	}
    else
    {
        labelCompanionUsed.text = [NSString stringWithFormat:@"Companion used: None"];
    }
...
- (IBAction)buttonSetCompanion:(id)sender {
    CompanionSelectionViewController *selectCompanionView = [[[CompanionSelectionViewController alloc] initWithNibName:nil bundle:nil] autorelease];
    //[self presentModalViewController:selectCompanionView animated:YES];
    [self.navigationController pushViewController:selectCompanionView animated:YES];
}
