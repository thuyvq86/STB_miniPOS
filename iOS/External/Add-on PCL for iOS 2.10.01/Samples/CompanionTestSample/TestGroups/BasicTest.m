...
-(void)logSerialData:(NSData *)data incomming:(BOOL)isIncoming {
	NSDateFormatter * dateFormatter = [[NSDateFormatter alloc] init];
	[dateFormatter setDateFormat:@"yyyy-MM-dd HH:mm:ss.SSS"];
	NSString * log = [NSString stringWithFormat:@"%@ [Data: %@, Length: %lu]\r\n\t", [dateFormatter stringFromDate:[NSDate date]], (isIncoming==YES?@"iSMP->iPhone":@"iPhone->iSMP"), (unsigned long)[data length]];
...
-(UITextView *)addTextViewWithTitle:(NSString *)title {
    UILabel	* label = [[[UILabel alloc] initWithFrame:CGRectMake(kHMargin, _vCursor, scrollView.frame.size.width/2- kHMargin*2, kLabelHeight)] autorelease];
    label.text = title;
    label.font = [UIFont systemFontOfSize:13];
    label.backgroundColor = [UIColor clearColor];
    //	_vCursor += kLabelHeight;
    UITextView * texteView = [[[UITextView alloc] initWithFrame:CGRectMake(scrollView.frame.size.width/2 - kHMargin, _vCursor, scrollView.frame.size.width/2 - kHMargin*2, kTextFieldHeight*4)] autorelease];
    _vCursor += kLabelHeight;
    [scrollView addSubview:label];
    [scrollView addSubview:texteView];
    [scrollView setContentSize:CGSizeMake(scrollView.frame.size.width, _vCursor)];
    return texteView;
}
