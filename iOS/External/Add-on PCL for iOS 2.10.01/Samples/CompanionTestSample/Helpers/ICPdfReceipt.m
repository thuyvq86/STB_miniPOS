...
-(id)init {
	if ((self = [super init])) {
		...
		pdfTextSize				= 14;
		pdfTextAlignment		= (UITextAlignment) NSTextAlignmentLeft;
		_pdfContext				= NULL;
...
-(CGSize)getBitmapSizeWithImage:(UIImage *)image {
    ...
    CGRect canvas = {{kHorizontalMargin / scaleFactor, (kPdfPageHeight - kVerticalMargin) / scaleFactor - imageHeight}, {imageWidth, imageHeight}};
    return canvas.size;
}
