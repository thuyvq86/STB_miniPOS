...
-(void)getComponentsInfoHelper {
	...
	if (components == nil) {
		...
		for (ICSoftwareComponent * softwareComponent in components) {
			componentString = [NSString stringWithFormat:@"Name: %@ Version: %@ Type: %lu CRC: %@", softwareComponent.name, softwareComponent.version, (unsigned long)softwareComponent.type, softwareComponent.crc];
