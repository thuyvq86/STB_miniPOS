...
-(void)accessoryDidDisconnect:(ICISMPDevice *)sender {
	...
	[self logMessage:[NSString stringWithFormat:@"Battery Life: %ld Days, %ld Hours, %d Minutes, %d Seconds", 
					  (long)[conversionInfo day], (long)[conversionInfo hour], [conversionInfo minute], [conversionInfo second]]];
	[date1 release];
	[date2 release];
	self.stressCommunication	= NO;
	self.control				= nil;
