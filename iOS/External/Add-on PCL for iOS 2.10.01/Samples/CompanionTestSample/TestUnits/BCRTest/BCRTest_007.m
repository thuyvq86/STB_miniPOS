...
-(void)accessoryDidConnect:(ICISMPDevice *)sender {
    ...
    {
         NSLog(@"TRACE BCR RESET TERMINAL ARMED !!!!!!!!!!!");
        [self performSelector:@selector(resetTerminal) withObject:nil afterDelay:(10 + random() % 3)];
...
-(void)testHelper {
	...
	while (startTest) {
        NSLog(@"TRACE BCR powerOn");
		[self.barcodeReader powerOn];
        if (self.barcodeReader.isAvailable)
        {
            count_ok++;
             NSLog(@"TRACE BCR powerOn OK");
            [self.barcodeReader enableTrigger:YES];
             NSLog(@"TRACE BCR enable trigger");
            [NSThread sleepForTimeInterval:1.0];
            [self.barcodeReader powerOff];
             NSLog(@"TRACE BCR powerOff");
        }
        else
        {
             NSLog(@"TRACE BCR powerOn KO");
            count_ko++;
            [NSThread sleepForTimeInterval:1.0];
        }
