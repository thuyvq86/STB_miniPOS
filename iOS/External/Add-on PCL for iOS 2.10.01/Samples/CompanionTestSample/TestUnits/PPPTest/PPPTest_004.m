    ...
    if ([server.outputStream streamStatus] == NSStreamStatusOpen) {
        while (offset < messageLen) {
            ...
            if (len > 0) {
                ...
                [self logMessage:[NSString stringWithFormat:@"Server [%ld] Encountered an Error when sending the message", (long)serverIndex]];
                break;
            }
        }
    }
...
-(void)connectionEstablished:(ICTcpServer *)sender {
    ...
    [self logMessage:[NSString stringWithFormat:@"[%ld]Client Connected [Name: %@]", (long)serverIndex, sender.peerName]];
}
...
-(void)stream:(NSStream *)aStream handleEvent:(NSStreamEvent)eventCode {
    ...
    switch (eventCode) {
        ...
        case NSStreamEventHasBytesAvailable:
            ...
            if (serverIndex < NB_SERVER_INSTANCES) {
                ...
                if (len > 0) {
                    ...
                    if (message != nil) {
                        [self performSelectorOnMainThread:@selector(logMessage:) withObject:[NSString stringWithFormat:@"[%ld]Received: %@", (long)serverIndex, message] waitUntilDone:NO];
                    }
                    [message release];
                }
            }
        ...
        case NSStreamEventEndEncountered:
            ...
            [self performSelectorOnMainThread:@selector(logMessage:) withObject:[NSString stringWithFormat:@"[%ld]Tcp Server Streams did close", (long)serverIndex] waitUntilDone:NO];
            break;
        case NSStreamEventErrorOccurred:
            ...
            [self performSelectorOnMainThread:@selector(logMessage:) withObject:[NSString stringWithFormat:@"[%ld]Tcp Server Streams Encountered an Error", (long)serverIndex] waitUntilDone:NO];
            break;
